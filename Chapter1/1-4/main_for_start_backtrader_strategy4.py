...
class MovingAverageCrossStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        # )
        self.short_sma = bt.indicators.SMA( # is the same as bt.indicators.SimpleMovingAverage
            self.data.close, period=self.params.short_period
        )
        """20日均線作為長期移動平均線"""
        ...
        # )
        self.long_sma = bt.indicators.SMA( # is the same as bt.indicators.SimpleMovingAverage
            self.data.close, period=self.params.long_period
        )
        """初始化訂單狀態，用來追蹤當前的訂單"""
        self.order = None
    ...
    def notify_trade(self, trade):
        if trade.isclosed:
            print(f"這筆賺了多少錢（含手續費）: {trade.pnlcomm:.2f}")
...
"""
This means each trade case uses 90% of available cash at the time of the order, not a fixed amount.
So the posion size shrinks/grows with your portfolio value over time, but no new money is added externally.
PercentSizer controls how many shares to buy each time self.buy() is called. Without it, Backtrader defaults to buying 1 share per order.
With perents=90, each buy order uses 90% of available cash to determine the share count.

Why 90% and not 100%?
Leaving a 10% buffer avoids order rejections due to commission costs or rounding — if you use 100%, the commission fee may push the total cost slightly over your cash balance, causing the order to fail.
"""
