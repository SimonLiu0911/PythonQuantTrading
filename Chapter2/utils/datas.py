...
def download_all_date_data(flag):
    try:
        ...
        raise ImportError("akshare is required for download_all_date_data; install via pip if needed.")
    # 获取所有股票代码，akshare接口
...
def get_zz500_stocks(time):
    try:
        ...
        raise ImportError("baostock is required for get_zz500_stocks; install via pip if needed.")
    # 登陆系统
    lg = bs.login()
    # 显示登陆返回信息
...
def get_hs300_stocks(time):
    try:
        ...
        raise ImportError("baostock is required for get_hs300_stocks; install via pip if needed.")
    # 登陆系统
    lg = bs.login()
    # 显示登陆返回信息
    print('login respond error_code:'+lg.error_code)
