...
## 1.1
### 常見的六大重要財報因子
因子：一個用來解釋或預測股票報酬的特徵或指標，
#### 價值因子(P1-26)
價值因子反映了股票價格相對於公司價值的高低估情況，通常使用股價和公司價值的比值來評估
價值因子 = 股價/公司價值

常見的價值因子：淨值比(P/B)、股價盈餘比(P/E)、股價營收比(P/S)

#### 成長因子(P1-27)
用來評估公司未來增長的潛力。
常見的成長性評估：月增率(MoM)、年增率(YoY)
...
#### 動能因子(P1-28)
以「強者恆強，弱者恆弱」的原則的因子，例如過看表線量好，未來很可能繼續表現出色。

#### 風險因子(P1-29)
反應投資者所需承擔的風險程度。較高的風險表示投資風險較高，但其實不一定能帶來高回報，因為風險是指「未來報酬的不確定性」。
常見的風險因子：Beta值、債務比率
...
### Alphalens
Alphalens 的主要功能是幫助我們深入分析因子策略的表現，評估這些策略是否能在實際操作中實現超越大盤的收益。
透過 Alphalens，可以精確檢視各種因子對策略收益的影響，從而建構或優化投資策略，提升投資績效。
#### Alpha
獲利能力的指標: 衡量的是一個投資組合或股票在經過風險調整後，勝過市場基準（如大盤指數）的部分
Alpha > 0：表示該投資的表現優於其風險水準應有的報酬（即賺到了超額利潤）。
Alpha = 0：表示報酬完全符合市場風險的預期。
Alpha < 0：表示表現不如預期，未能跑贏市場。
#### 如何使用 Alphalens
Command: pip install alphalens-reloaded==0.4.4

step:
1. 準備數據：
  首先，需要準備好股價資料(yfinance)和因子資料(FinLab)，這些資料將作為後續分析的基礎
2. 整理數據：
  將取得的資料整理成符合 Alphalens 規定的格式，以便能夠進行有效分析
3. 計算因子數值與未來收益：
  使用 "get_clean_factor_and_forward_returns" 函式來計算因子數值及其對應的未來收益表。
  這一步會產生 Alphalens 後續分析所需的清理過的因子數據和收益數據
4. 生成因子分析報表：
  最後，使用 "create_full_tear_sheet" 函式來生成最終的因子分析報表，該報表提供完整的因子表現評估，幫助我們理解因子的有效性及其對投資收益的影響

*data form:
1.股價資料集(類型: pd.DataForm):
  索引(index): 時間(如日期)
  欄位(column): 多個股票代碼
2.因子資料集(類型:pd.Series, 使用MultiIndex):
  索引(index): 時間、股票代碼
  欄位(column): 單一因子名稱

概念：(P1-48)
1. quantile(分位數): 指將資料從小到大排序後，均分成若干組。常見的有五分位數或十分位數，這種方法讓投資者可以比較不同分位數的表現。(按排名分組)
  例如：將因子資料集分為五個分位數時，第一分位數包含因子值最低的 20% 股票，最後一分位數包含因子值最高的 20% 股票。投資者可以藉由分析不同分位數的結果，討論哪個分位數下的股票表現較好。
2. bin(等間隔分組): 根據資料的數值範圍進行平均間隔的分組。每一組的範圍是相等的，不同於「quantile」根據資料數量分組。因此「bin」是將數值平均切分，而「quantile」是根據資料數量分成等量組。(按數值區間分組)
3. group(分組):「group」用來根據某些屬性（如行業或市值）將資料進行分組，從而在具有相似特性的資產中進行分析。如果指定「group」，Alphalens 會在每個「group」中再進行「quantile」分組。例如：可以先按行業進行分組，然後在每個行業組內分別應用因子分析。這樣做的好處是能理解同一因子在不同行業環境下的表現是否一致。(按類別分組，如產業)

get_clean_factor_and_forward_returns參數：
  factor：因子資料集(格式為 MultiIndex 的 Series，索引第 0 層是日期、第 1 層是資產)
  prices：價格資料集(格式為 index 為日期、column 為資產 的寬表 DataFrame)
...
資訊係數(Information Coefficient, IC)是指「因子分數」跟「未來報酬」在「橫截面」上到底有沒有關係、關係多強，量化投資中衡量 因子預測能力 的核心指標。
...
#### 因子值與分位數
將因子值分成5個分位數，分位數值愈低代表因子值愈低，反之則愈高，我們儘量要找最低跟最高分位數，不要找中間的，因為當良好收益表現集中在中間分位數時，投資決策就會變困難。
理想組合：
1. 最佳投組為因子值最高的組合+最差投組為因子值最低的組合
2. 最佳投組為因子值最低的組合+最差投組為因子值最高的組合
...
### 主成分分析(P1-95)
step 1: 對因子衣料進行標準化
step 2: 進行主成分分析，生成多個「主成分」
  主要觀察三個表：主成分的資訊保留比例表、主成分係數表、主成分結果表
step 3: 將主成分套用 Alphalens 進行因子分析
step 4: 根據 Alphalens 分析結果制定策略
...
###### Backtrader 中 Position 對象的輸出：
Position Begin
表示一個交易區塊的開始。在 Backtrader 中，每當策略執行 next() 或發生交易時，它會記錄當前持倉的「物理」狀態  
...
Closed
為實現損益(Unrealized/Floating Profit and Loss)
* Closed 紀錄的並非「金錢金額」，而是「被抵消掉的倉位單位數」或是「基準點移轉量」。

Adjbase
調整後基價(Adjusted Base Price)。是 Backtrader 用來計算盈虧的核心數值。
當你進行部分平倉（例如買 10 股賣 5 股）時，系統會用這個 Adjbase 來重新計算剩餘部位的成本
...
###### Note:
1. 當短均線（如5日、10日線）大於長均線（如20日、60日線）被視為買入訊號，是因為這代表「短期買盤動能已超過長期平均成本」，市場趨勢由空轉多或確立多頭。這反映了近期投資人願意以較高價格買進，使短期成本上移，形成技術面的「黃金交叉」
...
1. < 1.0：普通，不夠理想。
2. 1.0 ~ 2.0：優異。
3. > 2.0：極其優秀，通常只有頂尖的對沖基金或高頻交易能長期維持。
